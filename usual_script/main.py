...
if __name__ == '__main__':
    ...
    positions, districts, emails, send_email, schedule_hours, schedule_days, benefit_federal = config_parser.get_values_from_config()

    html_parser = DiabetHtmlReportParser()

    table, new_table, result_array = html_parser.get_tables_from_html_positions(positions=positions, districts=districts, benefit_federal=benefit_federal)
    ...
    if send_email:
        report_sender.send_emails(emails=emails, new_table=new_table)
